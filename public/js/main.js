...
var require_CronField = __commonJS((exports) => {
  ...
  class CronField {
    ...
    #isWildcardValue() {
      ...
      return Array.from({ length: this.max - this.min + 1 }, (_, i2) => i2 + this.min).every((value) => this.#values.includes(value));
    }
  }
...
var require_luxon = __commonJS((exports) => {
  ...
  function listStuff(loc2, length, englishFn, intlFn) {
    const mode = loc2.listingMode();
    if (mode === "error") {
      return null;
    } else if (mode === "en") {
  ...
  function supportsFastNumbers(loc2) {
    if (loc2.numberingSystem && loc2.numberingSystem !== "latn") {
      return false;
    } else {
      return loc2.numberingSystem === "latn" || !loc2.locale || loc2.locale.startsWith("en") || getCachedIntResolvedOptions(loc2.locale).numberingSystem === "latn";
    }
  }
  ...
  function parseDigits(str2) {
    ...
    if (isNaN(value)) {
      ...
      for (let i2 = 0;i2 < str2.length; i2++) {
        const code4 = str2.charCodeAt(i2);
  ...
  function uncomputeOrdinal(year, ordinal) {
    const table3 = isLeapYear(year) ? leapLadder : nonLeapLadder, month0 = table3.findIndex((i2) => i2 < ordinal), day = ordinal - table3[month0];
    return {
      month: month0 + 1,
      day
    };
  ...
  function usesLocalWeekValues(obj, loc2) {
    ...
    if (hasLocaleWeekData) {
      ...
      return {
        minDaysInFirstWeek: loc2.getMinDaysInFirstWeek(),
        startOfWeek: loc2.getStartOfWeek()
      };
    } else {
      return {
  ...
  function hasInvalidWeekData(obj, minDaysInFirstWeek = 4, startOfWeek = 1) {
    const validYear = isInteger2(obj.weekYear), validWeek = integerBetween(obj.weekNumber, 1, weeksInWeekYear(obj.weekYear, minDaysInFirstWeek, startOfWeek)), validWeekday = integerBetween(obj.weekday, 1, 7);
    if (!validYear) {
  ...
  class Formatter {
    ...
    num(n2, p2 = 0, signDisplay = undefined) {
      if (this.opts.forceSimple) {
        return padStart2(n2, p2);
      }
      const opts = {
        ...this.opts
      };
    ...
    formatDateTimeFromString(dt2, fmt) {
      const knownEnglish = this.loc.listingMode() === "en", useDateTimeFormatter = this.loc.outputCalendar && this.loc.outputCalendar !== "gregory", string3 = (opts, extract) => this.loc.extract(dt2, opts, extract), formatOffset2 = (opts) => {
        if (dt2.isOffsetFixed && dt2.offset === 0 && opts.allowZ) {
          return "Z";
        }
        return dt2.isValid ? dt2.zone.formatOffset(dt2.ts, opts.format) : "";
      }, meridiem = () => knownEnglish ? meridiemForDateTime(dt2) : string3({
        hour: "numeric",
        hourCycle: "h12"
      }, "dayperiod"), month = (length, standalone) => knownEnglish ? monthForDateTime(dt2, length) : string3(standalone ? {
        month: length
      } : {
        month: length,
        day: "numeric"
      }, "month"), weekday = (length, standalone) => knownEnglish ? weekdayForDateTime(dt2, length) : string3(standalone ? {
        weekday: length
      } : {
        weekday: length,
        month: "long",
      ...
      }, era = (length) => knownEnglish ? eraForDateTime(dt2, length) : string3({
        era: length
        ...
        switch (token) {
          ...
          case "X":
            return this.num(Math.floor(dt2.ts / 1000));
          case "x":
  ...
  class Duration {
    ...
    static fromISO(text7, opts) {
      const [parsed] = parseISODuration(text7);
      if (parsed) {
        return Duration.fromObject(parsed, opts);
      } else {
        return Duration.invalid("unparsable", `the input "${text7}" can't be parsed as ISO 8601`);
      }
    }
    static fromISOTime(text7, opts) {
      const [parsed] = parseISOTimeOnly(text7);
      if (parsed) {
        return Duration.fromObject(parsed, opts);
      } else {
        return Duration.invalid("unparsable", `the input "${text7}" can't be parsed as ISO 8601`);
      ...
      for (const k of orderedUnits$1) {
        if (units.indexOf(k) >= 0) {
          ...
          for (const ak in accumulated) {
            own7 += this.matrix[ak][k] * accumulated[ak];
            accumulated[ak] = 0;
          }
          if (isNumber(vals[k])) {
            own7 += vals[k];
          }
          const i2 = Math.trunc(own7);
          built[k] = i2;
          accumulated[k] = (own7 * 1000 - i2 * 1000) / 1000;
  ...
  class Interval {
    ...
    static fromISO(text7, opts) {
      ...
      return Interval.invalid("unparsable", `the input "${text7}" can't be parsed as ISO 8601`);
    }
    static isInterval(o) {
  ...
  function unitForToken(token, loc2) {
    const one3 = digitRegex(loc2), two = digitRegex(loc2, "{2}"), three = digitRegex(loc2, "{3}"), four = digitRegex(loc2, "{4}"), six = digitRegex(loc2, "{6}"), oneOrTwo = digitRegex(loc2, "{1,2}"), oneToThree = digitRegex(loc2, "{1,3}"), oneToSix = digitRegex(loc2, "{1,6}"), oneToNine = digitRegex(loc2, "{1,9}"), twoToFour = digitRegex(loc2, "{2,4}"), fourToSix = digitRegex(loc2, "{4,6}"), literal = (t2) => ({
      ...
      switch (t2.val) {
        case "G":
          return oneOf(loc2.eras("short"), 0);
        case "GG":
          return oneOf(loc2.eras("long"), 0);
        case "y":
        ...
        case "MM":
          return intUnit(two);
        case "MMM":
          return oneOf(loc2.months("short", true), 1);
        case "MMMM":
          return oneOf(loc2.months("long", true), 1);
        case "L":
          return intUnit(oneOrTwo);
        case "LL":
          return intUnit(two);
        case "LLL":
          return oneOf(loc2.months("short", false), 1);
        case "LLLL":
          return oneOf(loc2.months("long", false), 1);
        case "d":
          return intUnit(oneOrTwo);
        case "dd":
          return intUnit(two);
        ...
        case "c":
          return intUnit(one3);
        case "EEE":
          return oneOf(loc2.weekdays("short", false), 1);
        case "EEEE":
          return oneOf(loc2.weekdays("long", false), 1);
        case "ccc":
          return oneOf(loc2.weekdays("short", true), 1);
        case "cccc":
          return oneOf(loc2.weekdays("long", true), 1);
        case "Z":
        case "ZZ":
  ...
  function match(input, regex, handlers2) {
    ...
    if (matches) {
      ...
      for (const i2 in handlers2) {
        if (hasOwnProperty2(handlers2, i2)) {
          const h = handlers2[i2], groups = h.groups ? h.groups + 1 : 1;
  ...
  function parseDataToDateTime(parsed, parsedZone, opts, format2, text7, specificOffset) {
    const {
      setZone,
      zone
    } = opts;
    if (parsed && Object.keys(parsed).length !== 0 || parsedZone) {
      ...
      return DateTime.invalid(new Invalid("unparsable", `the input "${text7}" can't be parsed as ${format2}`));
    }
  }
  function toTechFormat(dt2, format2, allowZ = true) {
    return dt2.isValid ? Formatter.create(Locale2.create("en-US"), {
      allowZ,
      forceSimple: true
  ...
  function quickDT(obj, opts) {
    ...
    const loc2 = Locale2.fromObject(opts);
    let ts, o;
  ...
  class DateTime {
    ...
    static fromObject(obj, opts = {}) {
      ...
      const {
        minDaysInFirstWeek,
        startOfWeek
      } = usesLocalWeekValues(normalized, loc2);
    ...
    static fromISO(text7, opts = {}) {
      const [vals, parsedZone] = parseISODate(text7);
      return parseDataToDateTime(vals, parsedZone, opts, "ISO 8601", text7);
    }
    static fromRFC2822(text7, opts = {}) {
      const [vals, parsedZone] = parseRFC2822Date(text7);
      return parseDataToDateTime(vals, parsedZone, opts, "RFC 2822", text7);
    }
    static fromHTTP(text7, opts = {}) {
      const [vals, parsedZone] = parseHTTPDate(text7);
      return parseDataToDateTime(vals, parsedZone, opts, "HTTP", opts);
    }
    static fromFormat(text7, fmt, opts = {}) {
      if (isUndefined(text7) || isUndefined(fmt)) {
        throw new InvalidArgumentError("fromFormat requires an input string and a format");
      }
      const {
        locale = null,
        numberingSystem = null
      } = opts, localeToUse = Locale2.fromOpts({
        locale,
        numberingSystem,
        defaultToEN: true
      }), [vals, parsedZone, specificOffset, invalid2] = parseFromTokens(localeToUse, text7, fmt);
      if (invalid2) {
        return DateTime.invalid(invalid2);
      } else {
        return parseDataToDateTime(vals, parsedZone, opts, `format ${fmt}`, text7, specificOffset);
      }
    }
    static fromString(text7, fmt, opts = {}) {
      return DateTime.fromFormat(text7, fmt, opts);
    }
    static fromSQL(text7, opts = {}) {
      const [vals, parsedZone] = parseSQL(text7);
      return parseDataToDateTime(vals, parsedZone, opts, "SQL", text7);
    }
    static invalid(reason, explanation = null) {
      if (!reason) {
    ...
    static parseFormatForOpts(formatOpts, localeOpts = {}) {
      const tokenList = formatOptsToTokens(formatOpts, Locale2.fromObject(localeOpts));
      return !tokenList ? null : tokenList.map((t2) => t2 ? t2.val : null).join("");
    }
    static expandFormat(fmt, localeOpts = {}) {
      const expanded = expandMacroTokens(Formatter.parseFormat(fmt), Locale2.fromObject(localeOpts));
    ...
    static fromFormatExplain(text7, fmt, options = {}) {
      const {
        locale = null,
        numberingSystem = null
      } = options, localeToUse = Locale2.fromOpts({
        locale,
        numberingSystem,
        defaultToEN: true
      });
      return explainFromTokens(localeToUse, text7, fmt);
    }
    static fromStringExplain(text7, fmt, options = {}) {
      return DateTime.fromFormatExplain(text7, fmt, options);
    }
    ...
    static fromFormatParser(text7, formatParser, opts = {}) {
      if (isUndefined(text7) || isUndefined(formatParser)) {
      ...
      const {
        ...
        specificOffset,
        invalidReason
      } = formatParser.explainFromTokens(text7);
      if (invalidReason) {
        return DateTime.invalid(invalidReason);
      } else {
        return parseDataToDateTime(result, zone, opts, `format ${formatParser.format}`, text7, specificOffset);
      }
    }
...
var require_CronFieldCollection = __commonJS((exports) => {
  ...
  class CronFieldCollection {
    ...
    static compactField(input) {
      ...
      input.forEach((item, i2, arr) => {
        if (current === undefined) {
          current = { start: item, count: 1 };
          return;
        }
        const prevItem = arr[i2 - 1] || current.start;
        const nextItem = arr[i2 + 1];
        if (item === "L" || item === "W") {
          output.push(current);
...
var require_CronExpression = __commonJS((exports) => {
  ...
  class CronExpression {
    ...
    static #matchSchedule(value, sequence) {
      return sequence.some((element3) => element3 === value);
    }
    ...
    includesDate(date) {
      ...
      const dt2 = new CronDate_1.CronDate(date, this.#tz);
...
var require_random = __commonJS((exports) => {
  ...
  function xfnv1a(str2) {
    let h = 2166136261 >>> 0;
    for (let i2 = 0;i2 < str2.length; i2++) {
      h ^= str2.charCodeAt(i2);
  ...
  function seededRandom(str2) {
    const seed = str2 ? xfnv1a(str2)() : Math.floor(Math.random() * 10000000000);
    ...
    static #parseHashed(value, constraints, rand) {
      ...
      return value.replace(/H(?:\((\d+)-(\d+)\))?(?:\/(\d+))?/g, (_, min, max, step) => {
        if (min && max && step) {
          ...
          const values2 = [];
          for (let i2 = Math.floor(minStart / stepNum) * stepNum + offset;i2 <= maxNum; i2 += stepNum) {
            if (i2 >= minStart) {
              values2.push(i2);
            }
          }
          ...
          const offset = Math.floor(randomValue * stepNum);
          const values2 = [];
          for (let i2 = Math.floor(constraints.min / stepNum) * stepNum + offset;i2 <= constraints.max; i2 += stepNum) {
            if (i2 >= constraints.min) {
              values2.push(i2);
            }
          }
          return values2.join(",");
...
var require_CronFileParser = __commonJS((exports) => {
  ...
  class CronFileParser {
    static async parseFile(filePath) {
      const { readFile } = await Promise.resolve().then(() => __importStar((() => ({}))));
...
`);
      ...
      for (const block of blocks) {
        ...
        if (variableMatch) {
          const [, key2, value] = variableMatch;
          result.variables[key2] = value.replace(/["']/g, "");
          continue;
        }
        try {
...
function findAndReplace(tree, list2, options) {
  ...
  function handler(node2, parents) {
    ...
    const replace = pairs2[pairIndex][1];
    let start = 0;
    const siblings = parent.children;
...
function formatHeadingAsSetext(node2, state) {
  ...
  return Boolean((!node2.depth || node2.depth < 3) && toString2(node2) && (state.options.setext || literalWithBreak));
}
...
function linkReference(node2, _, state, info) {
  ...
  state.stack = stack;
  exit2();
  if (type2 === "full" || !text3 || text3 !== reference) {
    value += tracker.move(reference + "]");
  } else if (type2 === "shortcut") {
    value = value.slice(0, -1);
  } else {
...
function tokenizeTable(effects, ok3, nok) {
  ...
  function start(code3) {
    ...
    while (index2 > -1) {
      ...
      if (type2 === types2.lineEnding || type2 === types2.linePrefix)
        index2--;
      else
        break;
    }
  ...
  function headRowBreak(code3) {
    ...
    if (markdownSpace(code3)) {
      return factorySpace(effects, headRowBreak, types2.whitespace)(code3);
    }
    sizeB += 1;
    if (seen) {
      seen = false;
  ...
  function headDelimiterStart(code3) {
    ...
    if (markdownSpace(code3)) {
      ...
      return factorySpace(effects, headDelimiterBefore, types2.linePrefix, self2.parser.constructs.disable.null.includes("codeIndented") ? undefined : constants.tabSize)(code3);
    }
  ...
  function headDelimiterCellBefore(code3) {
    if (markdownSpace(code3)) {
      return factorySpace(effects, headDelimiterValueBefore, types2.whitespace)(code3);
    }
    return headDelimiterValueBefore(code3);
  }
  ...
  function headDelimiterRightAlignmentAfter(code3) {
    if (markdownSpace(code3)) {
      return factorySpace(effects, headDelimiterCellAfter, types2.whitespace)(code3);
    }
    return headDelimiterCellAfter(code3);
  }
  ...
  function bodyRowBreak(code3) {
    ...
    if (markdownSpace(code3)) {
      return factorySpace(effects, bodyRowBreak, types2.whitespace)(code3);
    }
    effects.enter(types2.data);
...
var serialize2 = (value, { json: json2, lossy } = {}) => {
  const _ = [];
  return serializer(!(json2 || lossy), !!json2, new Map, _)(value), _;
};
...
function createState(tree, options) {
  ...
  function one2(node2, parent) {
    ...
    if (own4.call(state.handlers, type2) && handle2) {
...
` + this.finalizeText(texte).trim();
  ...
  timer;
  finalizeText(text7) {
    text7 = text7.replace(/\bhlink\((.+?)\)/g, (_tout, args) => {
  ...
  write(text7) {
    this.content.insertAdjacentHTML("beforeend", text7);
    return true;
  }
  isOpened() {
...
class Editing {
  ...
  createNewTask(work) {
    ...
    if (work.cron) {
      this.showNextTime(owork.querySelector("input.form-work-cron"));
    }
    return owork;
  }
  setFormId(form, work) {
  ...
  showNextTime(field) {
    ...
    try {
      ...
      span2.innerHTML = `${jour} ${heure}`;
    } catch (err) {
      span2.classList.add("error");
      span2.innerHTML = "-bad cron-";
    }
  }
  getDefaultId() {
