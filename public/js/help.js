...
var require_ipc = __commonJS((exports, module) => {
  ...
  function serializeFn({
    ...
    if (data instanceof Map) {
      return new Map(Array.from(data).map(([key2, value]) => [
        serializeFn({ data: key2, depth: actualDepth - 1, seen }),
    ...
    seen.add(data);
    return Object.fromEntries(Object.entries(data).map(([key2, value]) => [
      key2,
      serializeFn({ data: value, depth: actualDepth - 1, seen })
    ]));
  }
